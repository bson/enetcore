...
bool Dhcp::Receive(IOBuffer* buf)
{
	...
	if (iph.proto != IPPROTO_UDP || sport != SERVER_PORT || dport != CLIENT_PORT
        || pkt->op != BOOTREPLY) {
        //DMSG("DHCP: not DHCP BOOTREPLY, proto %d, sport %d, dport %d", 
        //iph.proto, sport, dport);
		return false;
	}
	...
	if (pkt->xid == _xid) {
		...
		default:
            DMSG("DHCP: unknown state %d", _state);
		}
	}
...
Time Dhcp::Service()
{
    ...
    if (!_netif.GetLinkStatus()) {
        //DMSG("DHCP: link down, doing nothing");
        _backoff = 1;
        Backoff();
...
IOBuffer* Dhcp::AllocPacket()
{
	...
	pkt->secs = Htons((Time::Now() - _start).GetSec());
    pkt->flags = 0;
