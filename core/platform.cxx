...
void* Region::GetAlignedMem(int amount, uint alignment)
{
    ScopedNoInt G;
    ...
    uint8_t* next = (uint8_t*)Util::Align((uintptr_t)_next, uintptr_t(alignment));

	if (amount > 0) {
		if (next + amount >= _end - _reserve)
            return (void*)-1;

// To zero out memory
//		::memset(next, 0, amount);

		return exch<uint8_t*>(_next, next + amount);
	}
