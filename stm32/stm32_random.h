...
class Stm32Random {
    ...
    template typename <T>
    [[_finline]] static T& reg(const Register r) {
        return *((T*)(BASE_RNG + (uint32_t)r)); 
    }

    // Start RNG generator.  Returns true if okay.
    static bool Init() {
        volatile uint32_t& cr = reg<volatile uint32_t>(RNG_CR);
        cr &= BIT(IE);
        cr |= BIT(RNGEN);
        (void)Random();

        volatile uint32_t& sr = reg<volatile uint32_t>(RNG_SR);
    ...
    static uint32_t Random() {
        ...
        while (!(sr & BIT(DRDY)))
            ;
        return reg<volatile uint32_t>(RNG_DR);
    }
