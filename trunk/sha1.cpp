...
void Sha1::Final(uint8_t digest[Sha1::DIGEST_SIZE])
{
    uint32_t i;
	...
	Sha1::Transform(_state, _buffer);
#endif
}


void Sha1::Hash(const void* block, uint len, uint8_t digest[Sha1::DIGEST_SIZE])
{
	Sha1 sha1;
	sha1.Update((const uint8_t*)block, len);
	sha1.Final(digest);
}
